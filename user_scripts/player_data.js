...
save = async function(player) {
    try {
        ...
        log("\x1b[36mplayer_data.js", "save", `Saved player data for ${player.username} (${player.userId})`)
    } catch (error) {
        log("\x1b[36mplayer_data.js", "save", `Failed to save player data for ${player.username} (${player.userId}): ${error}`, 3)
    }
}

saveAll = async function() {
    log("\x1b[36mplayer_data.js", "saveAll", "Saving player data for every player...", 1)
    for (let i = 0; i < Game.players.length; i++) {
        await save(Game.players[i])
    }
    log("\x1b[36mplayer_data.js", "saveAll", "Saved player data for all players")
}
...
wipe = async function(player) {
    try {
        ...
        log("\x1b[36mplayer_data.js", "wipe", `Player data has been DELETED for ${player.username} (${player.userId})!!!`, 2)
    } catch (error) {
        log("\x1b[36mplayer_data.js", "wipe", `Failed to delete player data for ${player.username} (${player.userId}): ${error}`, 3)
    }
}
...
Game.on("playerJoin", async (player) => {
    ...
    let playerData = await Player.findOne({ userId: player.userId }).catch((error) => {
        ...
        log("\x1b[36mplayer_data.js", "Game.on(\"playerJoin\")", `Failed to retreive player data for ${player.username} (${player.userId})`, 3)
    })
    ...
    if (playerData) {
        ...
        if (playerData.version == currentVersion) {
            ...
            try {
                ...
                await f(player)
                log("\x1b[36mplayer_data.js", "Game.on(\"playerJoin\")", `Updated player data for ${player.username} (${player.userId})`)
                if (player.data.version < currentVersion) update(player)
            } catch (error) {
                log("\x1b[36mplayer_data.js", "Game.on(\"playerJoin\")", `Failed to update player data for ${player.username} (${player.userId}): ${error}`, 3)
            }
        }
        log("\x1b[36mplayer_data.js", "Game.on(\"playerJoin\")", `Loaded player data for ${player.username} (${player.userId}) (v${player.data.version})`)
    } else {
        //New player
        let playerData = new Player()
        playerData.userId = player.userId
        ...
        await playerData.save()
        player.data = playerData
        log("\x1b[36mplayer_data.js", "Game.on(\"playerJoin\")", `Created player data for ${player.username} (${player.userId}) (v${player.data.version})`)
    }
